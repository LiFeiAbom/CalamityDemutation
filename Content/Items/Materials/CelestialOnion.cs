...
namespace CalamityDemutation.Content.Items.Materials
{
    ...
    /// </summary>
    internal class CelestialOnion : ModItem
    {
        /// <summary>
        /// 物品基础属性：尺寸、专家标记、堆叠上限与使用姿势/音效
        /// </summary>
        ...
        /// <summary>
        /// 使用前置判定：仅在专家/大师模式且尚未解锁额外饰品栏时可使用。
        /// 返回 false 时物品不会被消耗，也不会进入 UseItem。
        /// </summary>
        public override bool CanUseItem(Player player)
        {
            CalamityDemutationPlayer modPlayer = player.GetModPlayer<CalamityDemutationPlayer>();
            // 仅在专家/大师模式，且尚未开启额外饰品栏时可用
            if (!Main.expertMode || modPlayer.extraAccessoryML)   // Main.expertMode 同时覆盖专家与大师；extraAccessoryML 为“已使用天界洋葱”标志
            {
        ...
        /// <summary>
        /// 执行解锁：处于使用动画且尚未解锁时，置位 extraAccessoryML 永久标志并同步该玩家状态给所有端。
        /// 标志随存档持久化（见 CalamityDemutationPlayer），由 CelestialOnionSlot 读取以显示额外饰品栏。
        /// </summary>
        public override bool? UseItem(Player player)
        {
            CalamityDemutationPlayer modPlayer = player.GetModPlayer<CalamityDemutationPlayer>();
            // 播放使用动画且尚未开启时执行：锁定使用冷却并开启额外饰品栏，再同步给其他玩家
    ...
    /// 天界洋葱提供的额外饰品栏
    /// </summary>
    internal class CelestialOnionSlot : ModAccessorySlot
    {
        /// <summary>
        /// 该额外饰品栏是否启用：玩家对象有效且已使用天界洋葱（extraAccessoryML）时启用。
        ...
        public override bool IsEnabled()
        {
            ...
            if (!Player.active)   // 玩家未激活（初始化早期）→ 直接禁用，避免索引越界
                return false;
            return Player.GetModPlayer<CalamityDemutationPlayer>().extraAccessoryML;   // 已解锁额外饰品栏才启用
        }
