...
namespace CalamityDemutation.Content.Projectiles.Melee
{
    ...
    /// 贴图复用 GodKiller.cs，因此 Texture 属性指向一代的贴图路径。
    /// </summary>
    internal class GodSlayerDart:ModProjectile
    {
        public bool initialized = false;   // 是否已完成首次初始化（用于只设置一次 timeLeft）
        public bool isMount = false;       // 标记处于挂载形态（仅作记录，实际分支以 ai[0] 判定）
        public Player Owner => Main.player[Projectile.owner];   // 弹幕归属玩家
        /// <summary>
        ...
        public int hasfirecount = 0;         // 挂载形态已发射的子飞镖数（上限 8）
        public int firedely = 2;             // 挂载形态发射倒计时，归 0 时发射并重置为 3
        public int mountdartfirerelay = 3;   // 计划中的发射间隔（声明未使用，实际在 MountDark 中硬编码为 3）
        /// <summary>
        ...
        public int randomstart = Main.rand.Next(0, 9);   // 每枚挂载弹幕各自的随机起始扇区偏移（0~8）
        /// <summary>
